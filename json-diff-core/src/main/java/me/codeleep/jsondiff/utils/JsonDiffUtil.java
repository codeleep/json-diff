...
public class JsonDiffUtil {
    ...
    public static boolean isPrimitiveType(Object obj){
        ...
        if(obj instanceof DiffJsonArray || obj instanceof DiffJsonObject){
            return false;
        }
     ...
     * 类型只分为基本类型和复杂类型。复杂类型其实也只有DiffJsonObject
     *  一共可以划分出四种可能
     * @return
     */
    public static Class<?> getArrayHandleClass(DiffJsonArray expect, DiffJsonArray actual) {
        ...
        for (Object item: expect.toArray()) {
    ...
    public static Class<?> parseItemClass(Object item) {
        ...
        if (item instanceof DiffJsonArray) {
            return MultidimensionalArrayHandle.class;
        }
        if (item instanceof DiffJsonObject) {
